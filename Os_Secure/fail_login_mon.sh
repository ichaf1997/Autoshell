...
white_list=(192.168.142.86) ## IP Arrary . notice that IP format is OK , use blank as separator if you want add more than one ip in white_list

[ ! -e "$mon_log" ] && touch $mon_log
echo 
echo -e "Time\t$(date +%F\ %T)" >> $mon_log
echo "Start monitoring ..." >> $mon_log
            ...
            if [ "$(cat $mon_log|grep "$(echo $line|cut -d ":" -f 2)"|wc -l)" == "0" ];then
                if [ "$white_list_enable" == "1" ];then
                   ...
                   if [ "$value" != "1" ];then
                        ...
                        echo "$(date +%F\ %T) - add $(echo $line|cut -d ":" -f 2) to iptables (failed retry = $(echo $line|cut -d ":" -f 1))" >> $mon_log
                   else
                        echo "$(date +%F\ %T) - add $(echo $line|cut -d ":" -f 2) is in the white_list (failed retry = $(echo $line|cut -d ":" -f 1)) without any handle." >> $mon_log   
                   fi   
                else
                   iptables -A INPUT -s $(echo $line|cut -d ":" -f 2) -j DROP >/dev/null 2>&1
                   echo "$(date +%F\ %T) - add $(echo $line|cut -d ":" -f 2) to iptables (failed retry = $(echo $line|cut -d ":" -f 1))" >> $mon_log
